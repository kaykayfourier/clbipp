...
function BatteryIcon({ className }: { className?: string }) {
  return (
    <svg
      ...
      aria-hidden="true"
    >
      <rect x="2" y="5" width="15" height="10" rx="2" stroke="#AAAAAA" strokeWidth="1.5" />
      <path d="M17 8v4" stroke="#AAAAAA" strokeWidth="1.5" strokeLinecap="round" />
      <rect x="4" y="7" width="9" height="6" rx="1" fill="#AAAAAA" fillOpacity="0.3" />
    </svg>
  );
}
    ...
    return (
      <button
        ...
        className={cn(
          ...
          "hover:bg-[#F8F5EE] active:bg-[#F0EDE6]",
      ...
      >
        {/* Battery icon bubble */}
        <span className="shrink-0 flex items-center justify-center w-10 h-10 rounded-[10px] bg-[#F8F5EE]">
          <BatteryIcon />
        </span>

        {/* Text block */}
        <span className="flex-1 min-w-0">
          <span className="block text-sm font-semibold text-[#111111] font-mono">
            {id}
          </span>
          <span className="block text-xs text-[#666666] truncate mt-0.5">
            {subtitle}
          </span>
        </span>
...
function ListRowSkeleton({ className }: { className?: string }) {
  return (
    ...
    >
      <div className="w-10 h-10 rounded-[10px] bg-[#E5E5E5] animate-pulse shrink-0" />
      <div className="flex-1 space-y-2">
        <div className="h-3 bg-[#E5E5E5] rounded-full animate-pulse w-24" />
        <div className="h-2.5 bg-[#E5E5E5] rounded-full animate-pulse w-36" />
      </div>
      <div className="h-6 w-20 bg-[#E5E5E5] rounded-full animate-pulse shrink-0" />
    </div>
  );
}
