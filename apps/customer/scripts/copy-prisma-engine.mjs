/**
 * Copy Prisma's query engine binary into apps/customer/src/generated/client
 * before `next build`.
 *
 ...
 * `<cwd>/src/generated/client` — on Vercel, /var/task/apps/customer/src/generated/client.
 *
...
if (!existsSync(src)) {
  console.error(
    `[copy-prisma-engine] no generated client at ${src}\n` +
      ...
      `\`npx turbo run build --filter=customer\` so ^db:generate runs first.`,
  )
  process.exit(1)
}
