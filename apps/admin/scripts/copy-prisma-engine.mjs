/**
 * Copy Prisma's query engine binary into apps/admin/src/generated/client
 * before `next build`.
 *
 ...
 * `<cwd>/src/generated/client` — on Vercel, /var/task/apps/admin/src/generated/client.
 *
...
if (!existsSync(src)) {
  console.error(
    `[copy-prisma-engine] no generated client at ${src}\n` +
      ...
      `\`npx turbo run build --filter=admin\` so ^db:generate runs first.`,
  )
  process.exit(1)
}
